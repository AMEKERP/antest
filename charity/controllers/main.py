...
class AddCase(http.Controller):
    ...
    def case_form1(self, **kw):
        ...
        nation_rec = request.env['res.country'].sudo().search([('name','in',['الكويت','المغرب','الجزائر','مصر','تونس','السودان','الصومال','السعودية','لبنان','سوريا','اليمن','غير محدد الجنسية'])])
    ...
    def create_webcase(self, case_id=None, **kw):
        ...
        six_months_period_after = (datetime.now() + relativedelta(months=1)).date()
        print("six_months_Period_before", six_months_period_before)
        print('every thing is ok ...', kw)
        # if request.env['charity.charity'].sudo().search([('case_id', '=', case_id),('create_date', '<', six_months_period_after)]):
        if request.env['charity.charity'].sudo().search([('case_id', '=', case_id),('create_date', '>', six_months_period_before)]):
